...
def create_timeseries_chart(df, y_column, title, x_column_name_in_df=None, y_axis_title=None, chart_type='bar'):
    ...
    try:
        ...
        df_for_plotting[y_column] = df_for_plotting[y_column] / 1_000_000
    ...
    try:
        # Use only "M" as y-axis label to avoid overlay, and short date format for x-axis
        # Remove axis and chart labels
        plot_labels = {
            y_column: "",
            x_label: ""
        }
        ...
        if chart_type == 'bar':
            fig = px.bar(df_cleaned, x=final_x_data, y=y_column, title="", labels=plot_labels)
        elif chart_type == 'line':
            fig = px.line(df_cleaned, x=final_x_data, y=y_column, title="", labels=plot_labels, markers=True)
        else:
            return {"error": f"Unsupported chart type: {chart_type}"}

        fig.update_layout(**common_layout_args)
        fig.update_yaxes(tickformat=",.0f", ticksuffix="M", title_standoff=20)
        if pd.api.types.is_datetime64_any_dtype(final_x_data):
            fig.update_xaxes(tickformat='%Y', tickangle=90, type='category', title_standoff=20)
        else:
            fig.update_xaxes(tickangle=90, title_standoff=20)
